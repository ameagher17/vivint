...
# ─── AVA identity ────────────────────────────────────────────────────────────
AGENT_API_NAME = "AVA_Voice_Agent"      # afkeynote's brand-new agent (0 real sessions)
AGENT_VERSION = "v2"
# Template api name MUST be populated on the agent participant, else the dashboard's
# day-bucketed aggregations (containment trend line) drop the rows and collapse to a
# single dot. Canonical Einstein Service Agent template (matches AiAgentType above).
AGENT_TEMPLATE = "SvcCopilotTmpl__EinsteinServiceAgent"
PLANNER_ID = "16jgL000002Q9Pt"          # active AVA_Voice_Agent_v2 GenAiPlannerDefinition (15-char)
...
SESSION_DURATION_SECONDS = 165          # every session's interaction span; 0 real sessions so avg = 165s (2:45)
EFFORT_BUMP = 0                         # no blend needed (no real sessions) — all sessions are exactly 165s
...
#   The tiles filter to agent api name = 'AVA_Voice_Agent' and use a FIXED-LOD step
#   formula. afkeynote's AVA_Voice_Agent has exactly ONE pre-existing real session
#   (id 9cecdd47…, dated 2026-09-13, in-window) with 0 interactions => step-less, so
#   it lands in the session COUNTD denominator only (+1 total, +0 contained, +0 to
#   effort/feedback/task-res). Synthetic must therefore total 997 so the dashboard
#   reads 998 total / 200 contained — matching vivint's displayed 998/200 exactly.
N_DEFLECTED = 168                       # -> CLOSED_ACTION  (resolved & contained)  = numerator of Resolution
N_ESCALATED = 797                       # -> CLOSED_TRANSFERRED (transfers)
N_ABANDONED = 32                        # -> CLOSED_USER_REQUEST (contained, not action)
#   synthetic: total=997, contained=200 (168 action + 32 user), transferred=797
#   + 1 real step-less session (denom only) => dashboard total=998, contained=200
#   Containment = 200/998 = 20.04% ; Resolution = 168/200 = 84.00%
TASKRES_TOTAL = 450                     # sessions given a TaskResolution content-quality category
TASKRES_FULL = 378                      # of those, FULLY_RESOLVED (rest UNRESOLVED)
#   0 existing rows (new agent) -> 378/450 = 84.00%
FEEDBACK_TOTAL = 489                    # interactions given GOOD/BAD feedback
FEEDBACK_GOOD = 445                     # real feedback rows for this agent = 0 (brand new),
...
N_CONTAINED = N_DEFLECTED + N_ABANDONED   # 200 = CLOSED_ACTION + CLOSED_USER_REQUEST
...
#   The containment line chart buckets sessions by day. Spread the synthetic sessions
#   evenly across EVERY day of the current-month-to-date window (Sep 2..Sep 13, incl.
#   today) so the line is a smooth ~20% trend with only minor day-to-day ups/downs and
#   NO spike at today — today (offset 12) carries a normal ~20% day that blends with the
#   1 real session. Offsets start at Sep 2 (not Sep 1) to stay clear of the month-start
#   boundary (a Sep 1 00:00 UTC row can underflow into August in a behind-UTC org).
#   Global outcome counts are held EXACTLY at 168/797/32 (largest-remainder
#   apportionment), so the headline MTD KPIs (20.04% / 84%) are unchanged.
_DAY_OFFSETS = list(range(1, 13))          # month_start+1..+12 => Sep 2..Sep 13 (today)
_TOTAL_W = [1.00, 1.02, 0.98, 1.03, 0.97, 1.01, 0.99, 1.02, 0.98, 1.01, 0.99, 1.00]
_RATE    = [0.200, 0.210, 0.190, 0.205, 0.195, 0.215, 0.185, 0.205, 0.195, 0.208, 0.192, 0.200]
...
def build(org_id, planner, user_ids, std, now):
    ...
    for i in range(N_SESSIONS):
        ...
        # headline KPIs aggregate exactly this batch. day_offset (1..12 => Sep 2..13) comes
        # from SEP_PLAN so the containment line spreads evenly across every day up to today.
        # Past days spread over hours 0..11; today (Sep 13) only uses hours already elapsed so
        # no session is stamped in the future, and a final clamp guarantees start < now.
        month_start = now.replace(day=1, hour=0, minute=0, second=0, microsecond=0)
        day_dt = month_start + _dt.timedelta(days=day_offset)
        if day_dt.date() == now.date():                       # today: only hours already elapsed
            max_h = max(1, now.hour)
            start = day_dt + _dt.timedelta(hours=(i % max_h), minutes=(i * 7) % 60)
        else:
            start = day_dt + _dt.timedelta(hours=(i % 12), minutes=(i * 7) % 60)
        if start >= now:                                      # safety: never in the future
            start = now - _dt.timedelta(minutes=5 + (i % 30))
        t0 = start
        ...
        for pid, role, obj_, part in [(agent_pid, "AGENT", "GenAiPlannerDefinition", planner),
            ...
            rows["AiAgentSessionParticipant"].append({
                ...
                "AiAgentVersionApiName": AGENT_VERSION, "AiAgentTemplateApiName": AGENT_TEMPLATE,
...
def cmd_verify(dc, org):
    ...
    ava = ("SELECT COUNT(DISTINCT p.ssot__AiAgentSessionId__c) FROM ssot__AiAgentSessionParticipant__dlm p "
           "WHERE p.ssot__AiAgentSessionParticipantRole__c='AGENT' AND p.ssot__AiAgentApiName__c='%s'" % AGENT_API_NAME)
